...
class CustomBasePermission(BasePermission):
    def has_object_permission(self, request, view, obj):
        ...
        if view.action == 'retrieve':
            ...
            if not request.user.has_perm(f'{permission_natural_key[1]}.{permission_natural_key[0]}'):
                raise AccessDenied(f'You don\'t have view permission for {self.model.__name__} !', request=request)
            ...
            if not request.user.has_perm(f'{permission_natural_key[1]}.{permission_natural_key[0]}'):
                raise AccessDenied(f'You don\'t have delete permission for {self.model.__name__} !', request=request)
        return True
    ...
    def has_permission(self, request, view):
        ...
        if view.action == 'list':
            ...
            if not request.user.has_perm(f'{permission_natural_key[1]}.{permission_natural_key[0]}'):
                raise AccessDenied(f'You don\'t have add permission for {self.model.__name__} ! {request.user.get_all_permissions()}', request=request)
        return True